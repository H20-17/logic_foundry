...
--   Uses the definition: S = {x | x=e₁ ∨ x=e₂ ∨ ... ∨ x=eₙ}
...
roster elems =
    let
        ...
        -- Template indices (start high to avoid clashes with potential element indices)
        s_idx = 0 -- Index for the set S itself (Bound by hX)
        x_idx = 1 -- Index for the element variable x (Bound by aX)

        -- Generate unique template indices for each element placeholder based on unique list
        elem_indices = [2 .. length uniqueSortedElems + 1]
        elemPlaceholders = Prelude.map X elem_indices

        -- Build the core disjunction using placeholders
        disjunction = buildDisjunction (X x_idx) elemPlaceholders

        -- Build the Hilbert template
        propTemplate = (X x_idx `In` X s_idx) :<->: disjunction
        quantifiedProp = aX x_idx propTemplate
        hilbertTemplate = hX s_idx quantifiedProp

        -- Create substitutions: map element placeholders to actual unique, sorted element terms
        elemSubs = zip elem_indices uniqueSortedElems

    -- Perform the substitution
    in objDeBrSubXs elemSubs hilbertTemplate
...
-- | Returns Maybe [element] on success. Matches the Hilbert/Forall/Bicond/Disjunction structure.
-- | Note: This parser does not handle the empty set case (Tupl []); it expects that to be parsed elsewhere.
parseRoster :: ObjDeBr -> Maybe [ObjDeBr]
parseRoster obj
    -- Attempt to parse the Hilbert structure
    = do
        -- 1. Match outer Hilbert: Hilbert p'
        (p', idx_S) <- parseHilbert obj -- idx_S for the roster set S

        -- 2. Match outer Forall: Forall prop_x
        (prop_x, idx_x) <- parseForall2 p' -- idx_x for element x

        -- 3. Match Biconditional: x_in_S :<->: disjunction_part
        (x_in_S, disjunction_part) <- parseBiconditional prop_x
        ...
        -- 5. Parse the RHS Disjunction using the helper
        --    The variable to match in equalities is Bound idx_x
        elems <- extractElemsFromDisjunction (Bound idx_x) disjunction_part

        -- 6. Sanity Check: Ensure extracted elements don't contain S index
...
bigUnion setS =
    let
        ...
        u_idx    = 3 -- Placeholder for the union set 'U' defined by hX

        -- Inner existential: ∃Y (Y ∈ S ∧ x ∈ Y)
        inner_exists = eX y_idx ((X y_idx `In` X setS_idx) :&&: (X x_idx `In` X y_idx))

        -- Core property: x ∈ U ↔ ∃Y (Y ∈ S ∧ x ∈ Y)
        propTemplate = (X x_idx `In` X u_idx) :<->: inner_exists

        -- Quantified property: ∀x P(x, U, S)
        quantifiedProp = aX x_idx propTemplate

        -- Hilbert term: hX U (∀x P(x, U, S))
        hilbertTemplate = hX u_idx quantifiedProp

    -- Substitute the actual setS into the template
    in objDeBrSubX setS_idx setS hilbertTemplate


-- | Constructs the ObjDeBr term representing the intersection of a set of sets S.
--   Requires S to be non-empty for standard definition.
--   ∩S = hX I (∀x (x ∈ I ↔ (∃Y(Y ∈ S) ∧ ∀Y (Y ∈ S → x ∈ Y))))
--   Note: Added ∃Y(Y ∈ S) to handle domain. If S is empty, ∃Y(...) is false.
...
bigIntersection setS =
    let
        ...
        setS_idx = 0 -- Placeholder for input setS
        x_idx    = 1 -- Placeholder for element 'x'
        y_idx    = 2 -- Placeholder for set 'Y' within S (used in forall)
        z_idx    = 3 -- Placeholder for set 'Z' within S (used in exists for non-empty check)
        i_idx    = 4 -- Placeholder for the intersection set 'I' defined by hX
        ...
        -- Core property: x ∈ I ↔ (NonEmptyCheck ∧ InnerForall)
        propTemplate = (X x_idx `In` X i_idx) :<->: (non_empty_check :&&: inner_forall)

        -- Quantified property: ∀x P(x, I, S)
        quantifiedProp = aX x_idx propTemplate

        -- Hilbert term: hX I (∀x P(x, I, S))
        hilbertTemplate = hX i_idx quantifiedProp
...
parseBigUnion obj = do
    -- 1. Match outer Hilbert: Hilbert p'
    (p', idx_U) <- parseHilbert obj -- idx_U for the Union set U

    -- 2. Match outer Forall: Forall prop_x
    (prop_x, idx_x) <- parseForall2 p' -- idx_x for element x

    -- 3. Match Biconditional: x_in_U :<->: exists_Y_part
    (x_in_U, exists_Y_part) <- parseBiconditional prop_x

    -- 4. Check LHS: bound_x_lhs `In` bound_U_lhs
    (bound_x_lhs, bound_U_lhs) <- parseIn x_in_U
    bound_x_idx_lhs <- parseBound bound_x_lhs
    bound_U_idx_lhs <- parseBound bound_U_lhs
    guard (bound_x_idx_lhs == idx_x)
    guard (bound_U_idx_lhs == idx_U)

    -- 5. Parse RHS Exists: Exists y_in_S_and_x_in_Y
    (y_in_S_and_x_in_Y, idx_y) <- parseExists exists_Y_part -- idx_y for set Y

    -- 6. Parse inner Conjunction: y_in_S :&&: x_in_Y
    (y_in_S, x_in_Y) <- parseConjunction y_in_S_and_x_in_Y

    -- 7. Parse y_in_S: bound_y_lhs `In` setS
    (bound_y_lhs, setS) <- parseIn y_in_S
    bound_y_idx_lhs <- parseBound bound_y_lhs
    guard (bound_y_idx_lhs == idx_y)

    -- 8. Parse x_in_Y: bound_x_rhs `In` bound_y_rhs
    (bound_x_rhs, bound_y_rhs) <- parseIn x_in_Y
    ...
    -- 9. Sanity checks
    guard (not (objDeBrBoundVarInside setS idx_x || objDeBrBoundVarInside setS idx_y || objDeBrBoundVarInside setS idx_U))
...
parseBigIntersection obj = do
    -- 1. Match outer Hilbert: Hilbert p'
    (p', idx_I) <- parseHilbert obj -- idx_I for Intersection set I

    -- 2. Match outer Forall: Forall prop_x
    (prop_x, idx_x) <- parseForall2 p' -- idx_x for element x

    -- 3. Match Biconditional: x_in_I :<->: conj_part
    (x_in_I, conj_part) <- parseBiconditional prop_x

    -- 4. Check LHS: bound_x_lhs `In` bound_I_lhs
    (bound_x_lhs, bound_I_lhs) <- parseIn x_in_I
    bound_x_idx_lhs <- parseBound bound_x_lhs
    bound_I_idx_lhs <- parseBound bound_I_lhs
    guard (bound_x_idx_lhs == idx_x)
    guard (bound_I_idx_lhs == idx_I)

    -- 5. Parse RHS Conjunction: non_empty_check :&&: forall_Y_part
    (non_empty_check, forall_Y_part) <- parseConjunction conj_part

    -- 6. Parse non_empty_check: Exists z_in_S
    (z_in_S, idx_z) <- parseExists non_empty_check -- idx_z for dummy Z
    (bound_z_lhs, setS_from_exists) <- parseIn z_in_S
    bound_z_idx_lhs <- parseBound bound_z_lhs
    guard (bound_z_idx_lhs == idx_z)

    -- 7. Parse forall_Y_part: Forall y_in_S_implies_x_in_Y
    (y_in_S_implies_x_in_Y, idx_y) <- parseForall2 forall_Y_part -- idx_y for set Y

    -- 8. Parse Implication: y_in_S :->: x_in_Y
    (y_in_S, x_in_Y) <- parseImplication y_in_S_implies_x_in_Y

    -- 9. Parse y_in_S: bound_y_lhs `In` setS_from_forall
    (bound_y_lhs, setS_from_forall) <- parseIn y_in_S
    bound_y_idx_lhs <- parseBound bound_y_lhs
    guard (bound_y_idx_lhs == idx_y)

    -- 10. Parse x_in_Y: bound_x_rhs `In` bound_y_rhs
    (bound_x_rhs, bound_y_rhs) <- parseIn x_in_Y
    bound_x_idx_rhs <- parseBound bound_x_rhs
    bound_y_idx_rhs <- parseBound bound_y_rhs
    guard (bound_x_idx_rhs == idx_x)
    ...
    -- 11. Check that setS from exists and forall match
    guard (setS_from_exists == setS_from_forall)
    let setS = setS_from_exists

    -- 12. Sanity checks
    guard (not (objDeBrBoundVarInside setS idx_x || objDeBrBoundVarInside setS idx_y || objDeBrBoundVarInside setS idx_z || objDeBrBoundVarInside setS idx_I))
...
--   Uses the definition: P(A) = {x | x ⊆ A}
--   Implemented as: hX P (∀x (x ∈ P ↔ x ⊆ A))
powerSet :: ObjDeBr -> ObjDeBr
powerSet setA =
    let
        -- Template indices
        setA_idx = 0 -- Placeholder for input setA
        x_idx    = 1 -- Placeholder for element 'x' (a potential subset)
        p_idx    = 2 -- Placeholder for the power set 'P' defined by hX
        ...
        -- Hilbert term: hX P (∀x P(x, P, A))
        hilbertTemplate = hX p_idx quantifiedProp

    -- Substitute the actual setA into the template
...
-- | Returns Maybe setA on success.
parsePowerSet :: ObjDeBr -> Maybe ObjDeBr
parsePowerSet obj = do
    -- 1. Match outer Hilbert: Hilbert p'
    (p', idx_P) <- parseHilbert obj -- idx_P for the power set P

    -- 2. Match outer Forall: Forall prop_x
    (prop_x, idx_x) <- parseForall2 p' -- idx_x for element x

    -- 3. Match Biconditional: x_in_P :<->: x_subset_A
    (x_in_P, x_subset_A) <- parseBiconditional prop_x

    -- 4. Check LHS: bound_x_lhs `In` bound_P_lhs
    (bound_x_lhs, bound_P_lhs) <- parseIn x_in_P
    bound_x_idx_lhs <- parseBound bound_x_lhs
    bound_P_idx_lhs <- parseBound bound_P_lhs
    guard (bound_x_idx_lhs == idx_x) -- Check 'x' index matches Forall binder
    guard (bound_P_idx_lhs == idx_P) -- Check 'P' index matches Hilbert binder

    -- 5. Parse the RHS using parseSubset: Expecting subset (Bound idx_x) setA
    (subsetElement, setA) <- parseSubset x_subset_A
    boundVar <- parseBound subsetElement
    guard (boundVar == idx_x) -- Ensure the element in the subset relation is the bound variable

    -- 6. Sanity Check: Ensure setA doesn't contain the bound variables
    guard (not (objDeBrBoundVarInside setA idx_x || objDeBrBoundVarInside setA idx_P))
